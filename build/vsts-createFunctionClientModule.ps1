...
$parentModule = 'MailForge'
if (-not $ModuleName) { $ModuleName = 'MailForge.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\MailForge'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\MailForge.psd1"
$prereqHash = @{
